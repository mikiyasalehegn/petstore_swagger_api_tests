from utils import (assert_status_code, assert_data_schema, assert_key, assert_pet_statuses, assert_error_messages,
                   GET_PET_SCHEMA, update_keys)
from data import PetsData


def test_create_pet_with_valid_data(pet_api):
    response = pet_api.create_pet(PetsData.CREATE_PET_WITH_VALID_DATA)
    assert_status_code(response, 201)
    assert_data_schema(response, GET_PET_SCHEMA.create_pet_schema)
...
def test_get_pet_by_correct_id(pet_api):
    ...
    assert_data_schema(response, GET_PET_SCHEMA.get_pet_schema)
...
def test_get_available_pets(pet_api):
    response = pet_api.get_pet(PetsData.FIND_BY_STATUS("available"))
    assert_status_code(response, 200)
    assert_pet_statuses(response, PetsData.AVAILABLE_PETS)

def test_get_pending_pets(pet_api):
    response = pet_api.get_pet(PetsData.FIND_BY_STATUS("pending"))
    assert_status_code(response, 200)
    assert_pet_statuses(response, PetsData.PENDING_PETS)

def test_get_sold_pets(pet_api):
    response = pet_api.get_pet(PetsData.FIND_BY_STATUS("sold"))
    assert_status_code(response, 200)
    assert_pet_statuses(response, PetsData.SOLD_PETS)

def test_get_pets_with_invalid_status(pet_api):
    response = pet_api.get_pet(PetsData.FIND_BY_STATUS(PetsData.INVALID_PET_STATUS))
    assert_status_code(response, 404)
    assert_data_schema(response, GET_PET_SCHEMA.error_message)
    assert_error_messages(response, "Invalid pet status")
